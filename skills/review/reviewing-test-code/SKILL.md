...
期待: Test Builderパターン適用可能性と保守性向上施策
```
...
期待: 包括的な品質評価、優先度付き改善計画、段階的実装ロードマップ
```
...
- **エージェント実行**: 5 つすべてのエージェントを並行実行（大規模プロジェクトでは時間短縮のため）
- **コーディネーター**: すべてのエージェント完了後に @agent-test-coordinator を実行
- **失敗時の処理**: エラーが発生した場合、その旨を報告し、取得できた結果のみでレポート生成
- **対象範囲**: ユーザーが指定したテストファイル/ディレクトリのみ（サブディレクトリは含まない、ただし明示的な指定があれば再帰的に処理）
...
- **チーム適合性**: チームのスキルレベルと利用可能なリソースを考慮した提案

### テスト品質基準

- **AAA構造**: Arrange-Act-Assert の明確な分離を推奨（ただし柔軟性も考慮）
- **Test Double**: 必要最小限の使用、過度なモック化を避ける
- **カバレッジ目標**: 重要なビジネスロジックは 80% 以上、全体は 70% 以上を推奨
- **型安全性**: テストコードでも型安全性を確保、any の使用は最小限に
