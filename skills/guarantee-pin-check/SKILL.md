...
Step C で `git diff` でなく `git status --porcelain` を使う理由: `git diff` は index との差分を見ないため、mutant を `git add` 済みのまま復元漏れがあっても空を返す。未追跡ファイルとして置いた mutant も見えない。この手順の主題は「テストが緑であることを保証の証拠にしない」であり、復元確認という安全装置自体が緑を偽ってはならない。
