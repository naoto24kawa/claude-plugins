...
- 人間への提示は通常の応答で行う。選択肢提示ツール（Claude Codeの `AskUserQuestion` など）がある環境では使ってよいが、**無い環境でも同じ手順が成立するようにする**。このスキルはskills CLI経由でCodexなどへも配布されるため、特定エージェント固有のツールを前提にしない。訂正候補の内容は必ず本文で示し、ツールは裁定（採用 / 却下 / 保留）の選択にだけ使う。
...
| プロダクト名 | 成果物ディレクトリ `<product-name>` に使う。対象文書があれば見出しから提案し、無ければ問う。kebab-caseへ正規化する |
...
常設12レンズをすべて採用し、`references/lens-catalog.md` にある条件付き7レンズの発動条件を判定する。条件付きレンズの該当・非該当は人間に確認し、エージェントが単独で「非該当」と決めない。
