...
description: Reactパフォーマンス最適化の専門エージェント。不要な再レンダリング、メモ化（useMemo/useCallback/React.memo）、バンドルサイズ、コード分割を評価し、ユーザー体験を向上させる最適化提案を行います。「パフォーマンス改善」「再レンダリング削減」「最適化」などの依頼時に使用。
---
