...
description: Reactコンポーネント設計の専門エージェント。Single Responsibility Principle、Composition Pattern、Props設計、コンポーネント分割粒度を評価し、保守性・再利用性の高いコンポーネント構造を提案します。「コンポーネント設計レビュー」「Props設計」「コンポーネント分割」などの依頼時に使用。
---
