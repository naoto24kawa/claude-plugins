...
description: Reactコンポーネントを6つの専門エージェント（コンポーネント設計、パフォーマンス、状態管理、アクセシビリティ、テスト容易性、shadcn/ui）で多角的にレビューし、トレードオフを考慮した優先度付き改善計画を策定する。Use when the user mentions "React review", "Reactレビュー", "コンポーネント品質", "パフォーマンス改善", "a11y確認", "accessibility", "shadcn", or needs React component quality verification.
---
...
- @agent-react-component-design-reviewer によるコンポーネント設計の評価
- @agent-react-performance-reviewer によるパフォーマンスの評価
- @agent-react-state-management-reviewer による状態管理の評価
- @agent-react-accessibility-reviewer によるアクセシビリティの評価
- @agent-react-testability-reviewer によるテスト容易性の評価
- @agent-react-shadcn-reviewer による shadcn/ui ベストプラクティスの評価
...
- 6つの専門エージェントを並行実行
  - @agent-react-component-design-reviewer
  - @agent-react-performance-reviewer
  - @agent-react-state-management-reviewer
  - @agent-react-accessibility-reviewer
  - @agent-react-testability-reviewer
  - @agent-react-shadcn-reviewer
