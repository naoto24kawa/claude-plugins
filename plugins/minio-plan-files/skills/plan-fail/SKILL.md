---
name: plan-fail
description: This skill should be used when the user asks to "planの処理が失敗した", "planを失敗にしたい", "processing→failedに移動したい", "plan-failを実行したい", "エラーを記録したい", "planを異常終了したい". processing/のplanをfailed/に移動するS3ディレクトリ移動のみのAgent側ワークフロー失敗ステップ。
user-invocable: true
---

processing/ のplanを failed/ に移動するワークフローの失敗ステップ。S3ディレクトリの移動のみを行う (SQSは関与しない)。
