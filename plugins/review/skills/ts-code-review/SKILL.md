---
name: ts-code-review
description: This skill should be used when the user asks to "code review", "コードレビューして", "コード品質を確認して", "改善提案して", "リファクタリングしたい", "設計を評価して", "型チェックして", "レビュー", "実装品質", or needs quality verification after implementation. Reviews implementation code with 6 specialized agents (SRP, readability, KISS, conventions, TypeScript, garbage detection) and creates prioritized improvement plans considering trade-offs.
context: fork
agents:
  - review-srp-reviewer
...
- Task ツール (subagent_type: `review-srp-reviewer`) による Single Responsibility Principle の評価
- Task ツール (subagent_type: `review-human-code-reviewer`) による Code for Humans 原則の評価
- Task ツール (subagent_type: `review-kiss-reviewer`) による KISS 原則 (シンプルさ) の評価
- Task ツール (subagent_type: `review-coc-reviewer`) による Convention over Configuration の評価
- Task ツール (subagent_type: `review-typescript-comprehensive`) による TypeScript 型安全性の総合評価
- Task ツール (subagent_type: `review-garbage-detector`) による不要ファイル・ゴミファイルの検出
...
- 各専門エージェントを並行または順次実行 (Task ツールの subagent_type で指定)
  - `review-srp-reviewer`
  - `review-human-code-reviewer`
  - `review-kiss-reviewer`
  - `review-coc-reviewer`
...
---
strictness: standard
exclude_paths: ["node_modules", "dist", "*.generated.ts"]
skip_agents: []
tradeoff_priority: ["security", "maintainability", "performance", "aesthetics"]
---
```
...
- **エージェント実行**: 6 つすべてのエージェントを並行実行 (`skip_agents` で除外可能)
...
- **トレードオフ判断**: セキュリティ > 保守性 > パフォーマンス > コード美観 の優先順位 (`tradeoff_priority` で変更可能)
