...
### 指摘のスコープ（AI_FIRST.md §3 準拠）

スペシャリストには「**correctness・明示要件・セキュリティに影響する指摘のみ報告**」も明示する。スコープ外（スタイル・好み・起きえないケースへの防御）は確信度が高くても報告対象外 — レビュアーは求めれば必ず何かを見つけるため、スコープ外まで修正すると過剰設計を生む。スコープ外指摘は optional 扱いとし、収束条件のカウントに含めない。
