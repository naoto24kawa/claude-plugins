...
**クエリパラメータ**: `ListQueryParams`
- `page`: ページ番号 (デフォルト: 1)
- `limit`: 1ページあたりの件数 (デフォルト: 20, 最大: 100)
- `sort`: ソート項目 (例: "createdAt", "-updatedAt")
- `filter`: フィルタ条件 (JSON文字列)

#### レスポンス

**成功時** (200 OK): `ListResponse<[Resource]>`
- `data[]`: [Resource]オブジェクトの配列
  - `id`: リソースID
  - `[field]`: [type]
...
**エラー時** (401 Unauthorized): `ErrorResponse`
...
**成功時** (200 OK): `[Resource]Response`
- `id`: リソースID
- `[field]`: [type]
- `createdAt`: 作成日時
- `updatedAt`: 更新日時

**エラー時** (404 Not Found): `ErrorResponse`
- `error.code`: "NOT_FOUND"
- `error.message`: "指定された[リソース]が見つかりません"
...
**ボディ** (application/json): `Create[Resource]Request`
- `[field]`: [type] - 必須、[説明]
- `[field]`: [type] - 任意、[説明]
...
**成功時** (201 Created): `[Resource]Response`
- `id`: リソースID
- `[field]`: [type]
- `createdAt`: 作成日時
- `updatedAt`: 更新日時

**エラー時** (400 Bad Request): `ValidationErrorResponse`
- `error.code`: "VALIDATION_ERROR"
- `error.message`: "入力内容に誤りがあります"
- `error.details[]`: フィールド別のエラー詳細
...
**ボディ** (application/json): `Update[Resource]Request`
- `[field]`: [type] - 任意、[説明]
...
すべてのエラーは `StandardErrorResponse` 形式で返却:

**フィールド:**
- `error.code`: エラーコード（大文字スネークケース）
- `error.message`: ユーザー向けエラーメッセージ
- `error.details[]`: 詳細情報（バリデーションエラー時、任意）
  - `field`: フィールド名
  - `message`: フィールド固有のエラーメッセージ
- `error.timestamp`: エラー発生時刻（ISO 8601）
- `error.requestId`: リクエストID（トレーシング用、任意）
